...
import { validate, ProcessAnalysisSchema } from "@/lib/validation";
import { createNotification, notifyAnalysisCompleted, notifyIfCriticalRisks } from "@/lib/notifications";

export async function POST(request: Request) {
  try {
    const v = validate(ProcessAnalysisSchema, await request.json());
    if (!v.success) return v.response;
    const { questionnaireId } = v.data;
    ...
    try {
      ...
      try { await import('@/lib/services/analysisLock').then(m => m.releaseAnalysisLock(String(questionnaire._id))); } catch { }
    }
    ...
    // Notify: analysis completed + critical risks (SSE + email)
